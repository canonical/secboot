...
// TestMeasureImageStartDriversAndAppsIncludesPostSeparatorAMDTSMEEvent
...
//
// The event must be included in the generated PCR2 profile because the firmware
// extends it into the TPM before launching the OS. Its expected digest is
// derived from the independently detected TSME status.
func (s *fwLoadHandlerSuite) TestMeasureImageStartDriversAndAppsIncludesPostSeparatorAMDTSMEEvent(c *C) {
	...
	params := LoadParams{AMDTSMEEnabledParamKey: true}
...
func (s *fwLoadHandlerSuite) TestMeasureImageStartDriversAndAppsRejectsTSMEStatusMismatch(c *C) {
	log := newLogWithPostSeparatorPCR2Event(c, &tcglog.Event{
		PCRIndex:  internal_efi.DriversAndAppsPCR,
	...
	err := NewFwLoadHandler(log).MeasureImageStart(ctx)
	c.Check(err, ErrorMatches, `cannot measure drivers and apps: AMD TSME configuration event data does not match expected TSME status \(expected 0x00, got 0x01\)`)
}

func (s *fwLoadHandlerSuite) TestMeasureImageStartDriversAndAppsRejectsUnknownPostSeparatorVendorEvent(c *C) {
	log := newLogWithPostSeparatorPCR2Event(c, &tcglog.Event{
		PCRIndex:  internal_efi.DriversAndAppsPCR,
		EventType: 0x00008402,
		Digests: tcglog.DigestMap{
			tpm2.HashAlgorithmSHA256: testutil.DecodeHexString(c, "4bf5122f344554c53bde2ebb8cd2b7e3d1600ad631c385a5d7cce23c7785459a"),
		},
		Data: tcglog.OpaqueEventData{0x01},
	...
	err := NewFwLoadHandler(log).MeasureImageStart(ctx)
	c.Check(err, ErrorMatches, `cannot measure drivers and apps: unexpected post-separator event type 00008402 found in PCR 2`)
}
