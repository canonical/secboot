...
`),
	...
	env := efitest.NewMockHostEnvironmentWithOpts(
		...
		efitest.WithAMD64Environment("GenuineIntel", 0x6, []uint64{internal_cpuid.FeatureSDBG, internal_cpuid.FeatureSMX}, 4, map[uint32]uint64{0xc80: 0x40000000}),
	)
	log := efitest.NewLog(c, &efitest.LogOptions{})
...
`),
	...
	env := efitest.NewMockHostEnvironmentWithOpts(
		efitest.WithSysfsDevices(devices...),
		efitest.WithAMD64Environment("GenuineIntel", 0x6, []uint64{internal_cpuid.FeatureSDBG}, 4, map[uint32]uint64{0xc80: 0x0}),
	)
	log := efitest.NewLog(c, &efitest.LogOptions{})
...
func (s *hostSecurityAMD64Suite) TestCheckDiscreteTPMPartialResetAttackMitigationStatusIntelNotDiscrete(c *C) {
	env := efitest.NewMockHostEnvironmentWithOpts(
		efitest.WithAMD64Environment("GenuineIntel", 0x6, []uint64{internal_cpuid.FeatureSMX}, 4, map[uint32]uint64{0x13a: (3 << 1)}),
	)
...
func (s *hostSecurityAMD64Suite) TestCheckDiscreteTPMPartialResetAttackMitigationStatusIntelUnavailableInvalidPCR0(c *C) {
	env := efitest.NewMockHostEnvironmentWithOpts(
		efitest.WithAMD64Environment("GenuineIntel", 0x6, []uint64{internal_cpuid.FeatureSMX}, 4, map[uint32]uint64{0x13a: (2 << 1)}),
	)
