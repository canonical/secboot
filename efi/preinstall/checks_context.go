...
func init() {
	errorKindToActions = map[ErrorKind][]Action{
		...
		ErrorKindTPMHierarchiesOwned: []Action{
			...
			// TODO: Add action to clear the authorization value.
		},
		...
		ErrorKindPCRUnsupported: []Action{
			ActionContactOSVendor, // suggest contacting the OS vendor, although we do already know which PCRs are unsupported
		},
		...
		ErrorKindVARSuppliedDriversPresent: []Action{
			// TODO: Add action to add PermitVARSuppliedDrivers to CheckFlags.
		...
		ErrorKindSysPrepApplicationsPresent: []Action{
			// TODO: Add action to add PermitSysPrepApplications to CheckFlags.
		...
		ErrorKindAbsolutePresent: []Action{
			...
			// TODO: Add action to add PermitAbsoluteComputrace to CheckFlags.
...
func (c *RunChecksContext) classifyRunChecksError(err error) (kind ErrorKind, args []any) {
	...
	if errors.As(err, &ownershipErr) {
		var args []TPMHierarchyOwnershipInfo
		...
		return ErrorKindTPMHierarchiesOwned, []any{args}
	}
	...
	if errors.Is(err, ErrTPMLockout) {
		...
		if err != nil {
			return ErrorKindTPMDeviceLockout, nil
		}
	...
	if errors.As(err, &pcrAlgErr) {
		...
		for _, err := range pcrAlgErr.BankErrs {
			...
			if !ok {
				// The error is not associated with a valid TPM response code,
				// but there could be other types of errors encountered when
				// executing TPM commands. Check for these too.
				if isInvalidTPMResponse(err) {
					// Some other error occurred, such as a transmission
					// error or in invalid response HMAC.
					// TODO: Test this case
					return ErrorKindUnexpectedTPMCommandFailure, nil
				}

				// The error is not a TPM error.
				continue
			}
