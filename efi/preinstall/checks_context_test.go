...
`),
	...
	c.Check(errs[1], ErrorMatches, `error with platform firmware \(PCR0\) measurements: PCR value mismatch \(actual from TPM 0x420bd3899738e6b41dccd18253a556e152e2b107559b89cbf0cbf1661ff6ee55, reconstructed from log 0xb0d6d5f50852be1524306ad88b928605c14338e56a1b8c0dc211a144524df2ef\) {"kind":"tpm-pcr-unusable","args":\[0\],"actions":\["contact-oem"\]}`)
	c.Check(errs[1], DeepEquals, NewErrorKindAndActions(ErrorKindPCRUnusable, []byte("[0]"), []Action{ActionContactOEM}, errs[1].Unwrap()))
}
...
`),
	...
	c.Assert(errs, HasLen, 1)
	c.Check(errs[0], ErrorMatches, `error with system security: encountered an error when checking Intel BootGuard configuration: no hardware root-of-trust properly configured: ME is in manufacturing mode: no firmware protections are enabled {"kind":"host-security","args":null,"actions":\["contact-oem"\]}`)
...
`),
	...
	c.Assert(errs, HasLen, 4)
	c.Check(errs[0], ErrorMatches, `error with boot manager code \(PCR4\) measurements: cannot verify the correctness of all EV_EFI_BOOT_SERVICES_APPLICATION boot manager launch event digests {"kind":"tpm-pcr-unusable","args":\[4\],"actions":\["contact-oem"\]}`)
