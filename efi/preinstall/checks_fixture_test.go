...
func runChecksHostFixturesFor(c *C, required runChecksHostCapabilities) []runChecksHostFixture {
	...
	if len(matches) == 0 {
		c.Error("no platform fixture provides the required host capabilities")
	}
	return matches
}
	...
	return []runChecksHostFixture{
		// amd64 fixtures
		{
			...
			capabilities: runChecksHostCapabilityValid |
				runChecksHostCapabilityNotVirtualMachine |
				runChecksHostCapabilityFirmwareTPM,
			environment:             efitest.WithAMD64Environment("GenuineIntel", 0x6, []uint64{internal_cpuid.FeatureSDBG, internal_cpuid.FeatureSMX}, 4, map[uint32]uint64{0xc80: 0x40000000, 0x13a: (3 << 1)}),
		...
		},
		{
			...
			capabilities: runChecksHostCapabilityValid |
				...
				runChecksHostCapabilityStartupLocality3InaccessibleFromOS |
				runChecksHostCapabilityStartupLocality4InaccessibleFromOS,
			environment:             efitest.WithAMD64Environment("GenuineIntel", 0x6, []uint64{internal_cpuid.FeatureSDBG, internal_cpuid.FeatureSMX}, 4, map[uint32]uint64{0xc80: 0x40000000, 0x13a: (2 << 1)}),
			virtualizationMode:      internal_efi.VirtModeNone,
			virtualizationDetection: internal_efi.DetectVirtModeAll,
			sysfsDevices:            intelDevices(fwStatusBase, true),
			arch:                    "amd64",
		},
		{
			...
			capabilities: runChecksHostCapabilityValid |
				...
				runChecksHostCapabilityStartupLocality3AccessibleFromOS |
				runChecksHostCapabilityStartupLocality4AccessibleFromOS,
			environment:             efitest.WithAMD64Environment("GenuineIntel", 0x6, []uint64{internal_cpuid.FeatureSDBG}, 4, map[uint32]uint64{0xc80: 0x40000000, 0x13a: (2 << 1)}),
			virtualizationMode:      internal_efi.VirtModeNone,
			virtualizationDetection: internal_efi.DetectVirtModeAll,
			sysfsDevices:            intelDevices(fwStatusBase, true),
			arch:                    "amd64",
		...
		},
		{
			...
			virtualizationMode:      internal_efi.VirtModeNone,
			virtualizationDetection: internal_efi.DetectVirtModeAll,
			sysfsDevices:            arm64Devices("tpm_crb", true),
			arch:                    "arm64",
		},
	}
}
