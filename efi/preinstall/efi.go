...
// github.com/canonical/go-efilib package), ("", false) will be returned.
func MakeEFIVariableAccessErrorArg(err error) (arg EFIVariableAccessErrorArg, ok bool) {
	...
	for efiErr, arg := range efiErrs {
		if !errors.Is(err, efiErr) {
			// The supplied error is not efiErr.
			continue
		}

		// The supplied error is an EFI variable access error (efiErr: arg).
		return arg, true
	}

	// The supplied error is not an EFI variable access error.
	return "", false
}
...
func isEFIVariableAccessError(err error) bool {
	_, ok := MakeEFIVariableAccessErrorArg(err)
	return ok
}
