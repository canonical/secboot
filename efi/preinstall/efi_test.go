...
func (*efiSuite) TestMakeEFIVariableAccessErrorArgVarsUnavailable(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(fmt.Errorf("some error: %w", efi.ErrVarsUnavailable))
	c.Check(ok, testutil.IsTrue)
	c.Check(arg, Equals, EFIVarsUnavailable)
}

func (*efiSuite) TestMakeEFIVariableAccessErrorArgVarNotExist(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(fmt.Errorf("some error: %w", efi.ErrVarNotExist))
	c.Check(ok, testutil.IsTrue)
	c.Check(arg, Equals, EFIVarNotExist)
}

func (*efiSuite) TestMakeEFIVariableAccessErrorArgInvalidParam(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(fmt.Errorf("some error: %w", efi.ErrVarInvalidParam))
	c.Check(ok, testutil.IsTrue)
	c.Check(arg, Equals, EFIVarInvalidParam)
}

func (*efiSuite) TestMakeEFIVariableAccessErrorArgVarDeviceError(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(fmt.Errorf("some error: %w", efi.ErrVarDeviceError))
	c.Check(ok, testutil.IsTrue)
	c.Check(arg, Equals, EFIVarDeviceError)
}

func (*efiSuite) TestMakeEFIVariableAccessErrorArgVarPermission(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(fmt.Errorf("some error: %w", efi.ErrVarPermission))
	c.Check(ok, testutil.IsTrue)
	c.Check(arg, Equals, EFIVarPermission)
}

func (*efiSuite) TestMakeEFIVariableAccessErrorArgVarInsufficientSpace(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(fmt.Errorf("some error: %w", efi.ErrVarInsufficientSpace))
	c.Check(ok, testutil.IsTrue)
	c.Check(arg, Equals, EFIVarInsufficientSpace)
}

func (*efiSuite) TestMakeEFIVariableAccessErrorArgVarWriteProtected(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(fmt.Errorf("some error: %w", efi.ErrVarWriteProtected))
	c.Check(ok, testutil.IsTrue)
	c.Check(arg, Equals, EFIVarWriteProtected)
}

func (*efiSuite) TestMakeEFIVariableAccessErrorArgInvalid(c *C) {
	arg, ok := MakeEFIVariableAccessErrorArg(errors.New("some error"))
	c.Check(ok, testutil.IsFalse)
	c.Check(arg, Equals, EFIVariableAccessErrorArg(""))
}
...
func (*efiSuite) TestEFIVariableAccessErrorArgErrorInvalid(c *C) {
	arg := EFIVariableAccessErrorArg("invalid")
	c.Check(arg.Error(), ErrorMatches, `unrecognized EFI variable access error code: "invalid"`)
}
