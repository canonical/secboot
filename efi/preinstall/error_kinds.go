...
const (
	...
	// set. This will be supplied with a single argument - a slice of
	...
	// resources. In most cases, this will be accompanied with 2 arguments
	// of type time.Duration, with the first argument being the maximum time
	// that it will take for the lockout to clear, and the second argument
	// being the maximum time it will take for the fail count to reach 0.
	// These arguments man not be present if another error occurred which
	// prevents them from being determined.
	...
	// will be accompanied with one argument of type []tpm2.HashAlgorithmId to
	// indicate which banks are broken.
	...
	// ErrorKindUnexpectedTPMCommandFailure indicates that an unexpected error occurred
	// whilst communicating with the TPM, such as a transmission error, which makes it
	...
	ErrorKindUnexpectedTPMCommandFailure ErrorKind = "unexpected-tpm-command-failure"
