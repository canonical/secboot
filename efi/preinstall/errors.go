...
var (
	...
	// ErrTPMLockoutAlreadyOwned is returned wrapped in TPM2DeviceError if the
	// authorization value for the lockout hierarchy is already set but the
	...
	// ErrUnsupportedTPMOwnership is returned wrapped in TPM2DeviceError if the
	// authorization value for the owner or endorsement hierarchies are set, which
	// currently isn't supported by snapd. Snapd needs the use of both of these
	// hierarchies, so if we want to support something other than snapd taking
	// ownership of these in the future, this will need coordination with snapd.
