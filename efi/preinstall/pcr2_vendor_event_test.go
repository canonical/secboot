...
// TestCheckDriversAndAppsMeasurementsAcceptsPostSeparatorAMDTSMEEvent confirms
// that the PCR2 preinstall check accepts the AMD TSME event before the OS launch
// boundary when it matches the independently detected status.
func (s *pcr2Suite) TestCheckDriversAndAppsMeasurementsAcceptsPostSeparatorAMDTSMEEvent(c *C) {
	...
	err := s.testCheckDriversAndAppsMeasurements(c, &testCheckDriversAndAppsMeasurementsParams{
		...
		amdConfig: &AMDPreOSMeasurementConfig{TSMEEnabled: true},
	})
	c.Check(err, IsNil)
}

func (s *pcr2Suite) TestCheckDriversAndAppsMeasurementsIgnoresPCR7AMDTSMEEvent(c *C) {
	err := s.testCheckDriversAndAppsMeasurements(c, &testCheckDriversAndAppsMeasurementsParams{
		env: efitest.NewMockHostEnvironmentWithOpts(
			efitest.WithMockVars(efitest.MockVars{}),
			efitest.WithLog(newLogWithAMDTSMEPCR7Event(c, false)),
		),
		pcrAlg:    tpm2.HashAlgorithmSHA256,
		amdConfig: &AMDPreOSMeasurementConfig{TSMEEnabled: false},
	})
	c.Check(err, IsNil)
}

func (s *pcr2Suite) TestCheckDriversAndAppsMeasurementsRejectsTSMEStatusMismatch(c *C) {
	log := newLogWithPostSeparatorPCR2Event(c, &tcglog.Event{
		PCRIndex:  internal_efi.DriversAndAppsPCR,
	...
	c.Check(err, ErrorMatches, `AMD TSME configuration event data does not match detected TSME status \(expected 0x00, got 0x01\)`)
}

func (s *pcr2Suite) TestCheckDriversAndAppsMeasurementsRejectsUnknownPostSeparatorVendorEvent(c *C) {
	log := newLogWithPostSeparatorPCR2Event(c, &tcglog.Event{
		PCRIndex:  internal_efi.DriversAndAppsPCR,
	...
	c.Check(err, ErrorMatches, `unexpected post-separator event type 00008402 found in PCR 2`)
}
