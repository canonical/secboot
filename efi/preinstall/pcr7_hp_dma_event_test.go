...
func (s *pcr7Suite) TestCheckSecureBootPolicyMeasurementsAndObtainAuthoritiesGoodWithHPPreBootDMAConfig(c *C) {
	err := s.testCheckSecureBootPolicyMeasurementsAndObtainAuthorities(c, &testCheckSecureBootPolicyMeasurementsAndObtainAuthoritiesParams{
		...
		amdConfig:     &AMDPreOSMeasurementConfig{TSMEEnabled: true, HPPreBootDMAConfigEnabled: true},
...
func (s *pcr7Suite) TestCheckSecureBootPolicyMeasurementsAndObtainAuthoritiesGoodWithDisabledTSME(c *C) {
	err := s.testCheckSecureBootPolicyMeasurementsAndObtainAuthorities(c, &testCheckSecureBootPolicyMeasurementsAndObtainAuthoritiesParams{
		env: efitest.NewMockHostEnvironmentWithOpts(
			efitest.WithMockVars(hpPreBootDMAConfigTestVars(c)),
...
func (s *pcr7Suite) TestCheckSecureBootPolicyMeasurementsAndObtainAuthoritiesRejectsMissingExpectedHPPreBootDMAConfig(c *C) {
	log := efitest.NewLog(c, &efitest.LogOptions{Algorithms: []tpm2.HashAlgorithmId{tpm2.HashAlgorithmSHA256}})
	err := s.checkHPPreBootDMAConfigLog(c, log)
	c.Check(err, ErrorMatches, `missing expected HP pre-boot DMA configuration event`)
}

func (s *pcr7Suite) TestCheckSecureBootPolicyMeasurementsAndObtainAuthoritiesRejectsUnexpectedHPPreBootDMAConfig(c *C) {
	err := s.testCheckSecureBootPolicyMeasurementsAndObtainAuthorities(c, &testCheckSecureBootPolicyMeasurementsAndObtainAuthoritiesParams{
		env: efitest.NewMockHostEnvironmentWithOpts(
			efitest.WithMockVars(hpPreBootDMAConfigTestVars(c)),
			efitest.WithLog(newLogWithHPPreBootDMAConfigEvent(c)),
