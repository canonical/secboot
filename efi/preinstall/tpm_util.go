...
// each member must be a modulus of 1 second.
func (a *TPMDeviceLockoutArgs) IsValid() bool {
	if a.IntervalDuration%time.Second != 0 {
		return false
	}
	return a.TotalDuration%time.Second == 0
}
...
// it must be a modulus of 1 second.
func (r TPMDeviceLockoutRecoveryArg) IsValid() bool {
	return time.Duration(r)%time.Second == 0
}
