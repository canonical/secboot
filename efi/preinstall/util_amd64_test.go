...
func (s *utilAmd64Suite) TestDetermineCPUVendorIntel(c *C) {
	env := efitest.NewMockHostEnvironmentWithOpts(efitest.WithAMD64Environment("GenuineIntel", 0x12, nil, 0, nil))
...
func (s *utilAmd64Suite) TestDetermineCPUVendorUnknown(c *C) {
	env := efitest.NewMockHostEnvironmentWithOpts(efitest.WithAMD64Environment("GenuineInte", 0x12, nil, 0, nil))
