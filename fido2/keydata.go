...
type keyData struct {
	...
	// SaltProvider is the name of the platform that provides the salt for this key data
	// SaltProvider string `json:"salt_provider"`
}

func newFIDO2ProtectedKey(authenticator *FIDO2Authenticator, providerName string, salt []byte, primaryKey secboot.PrimaryKey) (fkd *keyData, encryptedPayload []byte, primaryKeyOut secboot.PrimaryKey, unlockKey secboot.DiskUnlockKey, err error) {

	// TODO verify salt length and other salt requirements

	// 2. expand it once in order to hide the actual salt from a potential malicious FIDO2 authenticator
	hmacSecretSaltSize := 32

	fido2Key := make([]byte, hmacSecretSaltSize)
	r := hkdf.Expand(func() hash.Hash { return kdfAlg.New() }, salt, []byte("ubuntu-fde-fido2"))
	if _, err := io.ReadFull(r, fido2Key); err != nil {
		return nil, nil, nil, nil, fmt.Errorf("cannot derive encryption key: %w", err)
	}

	// 3. communicate with fido2 to get hmac-secret(salt)

	credentialID, hmacSecret, err := authenticator.MakeFDECredential(fido2Key)
	if err != nil {
		return nil, nil, nil, nil, fmt.Errorf("cannot create FIDO2 credential: %w", err)
	}

	// 4. Mix the result with the original symmetric key and expand once more to obtain the final symmetric key
	sym := make([]byte, hmacSecretSaltSize)
	r = hkdf.Expand(func() hash.Hash { return kdfAlg.New() }, salt, hmacSecret)
...
func NewFIDO2ProtectedKey(authenticator *FIDO2Authenticator, providerName string, salt []byte, primaryKey secboot.PrimaryKey) (protectedKey *secboot.KeyData, primaryKeyOut secboot.PrimaryKey, unlockKey secboot.DiskUnlockKey, err error) {
	fkd, fidoEncPayload, primaryKey, unlockKey, err := newFIDO2ProtectedKey(authenticator, providerName, salt, primaryKey)
	if err != nil {
...
func NewFIDO2ProtectedKeyWithSaltProvider(authenticator *FIDO2Authenticator, pkd *secboot.KeyData, primaryKey secboot.PrimaryKey) (protectedKey *secboot.KeyData, primaryKeyOut secboot.PrimaryKey, unlockKey secboot.DiskUnlockKey, err error) {
	providerName, providerPlatformKeyData, salt, err := pkd.RecoverFIDO2Salt()
	if err != nil {
		return nil, nil, nil, err
	}
