...
func recoverKeys(kd *keyData, encryptedPayload, salt []byte, authenticator *FIDO2Authenticator) ([]byte, error) {
	...
	hmacSecret, err := authenticator.GetHmacSecret(kd.CredentialID, fido2Key)
	if err != nil {
	...
	symB := make([]byte, hmacSecretSaltSize)
	r = hkdf.Expand(func() hash.Hash { return kdfAlg.New() }, salt, hmacSecret)
	if _, err := io.ReadFull(r, symB); err != nil {
		return nil, fmt.Errorf("cannot derive encryption key: %w", err)
	}
...
func RecoverKeys(data *secboot.PlatformKeyData, encryptedPayload, salt []byte, authenticator *FIDO2Authenticator) ([]byte, error) {
	var kd keyData
	...
	return recoverKeys(&kd, encryptedPayload, salt, authenticator)
}
...
var RecoverKeysWithFIDOProvider = func(providerName string, data *secboot.PlatformKeyData, encryptedPayload []byte, authenticator *FIDO2Authenticator) ([]byte, error) {
	...
	payload, err := recoverKeys(kd.Fido2, encryptedPayload, symA, authenticator)
	if err != nil {
		return nil, err
	}
