...
func (s *keyringSuite) TestReadKeyErr(c *C) {
	...
	c.Check(err, ErrorMatches, `cannot read key payload with buffer size 0: cannot complete operation because a specified key does not exist`)
}
...
func (s *keyringSuite) testGetKeyringID(c *C, id KeyID) error {
	// Test the GetKeyringID API and save the real ID. We
