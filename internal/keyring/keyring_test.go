...
func (s *keyringSuite) testAddKey(c *C, params *testAddKeyParams) error {
	...
	s.AddKeyToInvalidate(c, id, params.threadBound) // Make sure this key is invalidated during test tear-down.
...
func (s *keyringSuite) testReadKey(c *C, params *testReadKeyParams) error {
	...
	c.Assert(err, IsNil)
	s.AddKeyToInvalidate(c, KeyID(id), false) // Make sure this key is invalidated during test tear-down.
...
func (s *keyringSuite) testSearchKey(c *C, params *testSearchKeyParams) error {
	...
	c.Assert(err, IsNil)
	s.AddKeyToInvalidate(c, KeyID(expectedId), params.threadBound) // Make sure this key is invalidated during test tear-down.
...
func (s *keyringSuite) testGetKeyringID(c *C, id KeyID, threadBound bool) error {
	...
	c.Assert(err, IsNil)
	s.AddKeyToInvalidate(c, KeyID(keyId), threadBound) // Make sure this key is invalidated during test tear-down.
...
func (s *keyringSuite) TestGetKeyringID(c *C) {
	...
	c.Check(s.testGetKeyringID(c, ThreadKeyring, true), IsNil)
}

func (s *keyringSuite) TestGetKeyringIDDifferentID(c *C) {
	c.Check(s.testGetKeyringID(c, ProcessKeyring, false), IsNil)
}

func (s *keyringSuite) TestGetKeyringIDError(c *C) {
	c.Check(s.testGetKeyringID(c, KeyID(-25), false), Equals, ErrInvalidArgs)
}
...
func (s *keyringSuite) testLinkKey(c *C, params *testLinkKeyParams) error {
	...
	id, err := unix.AddKey(string(UserKeyType), "foo", []byte{1, 2, 3, 4}, int(params.creationKeyringId))
	c.Assert(err, IsNil)
	s.AddKeyToInvalidate(c, KeyID(id), params.threadBound) // Make sure this key is invalidated during test tear-down.

	// Test the LinkKey API.
	if err := LinkKey(KeyID(id), params.linkTargetKeyringId); err != nil {
		return err
	}

	s.CheckKeyInKeyring(c, KeyID(id), params.linkTargetKeyringId)
...
func (s *keyringSuite) testUnlinkKey(c *C, keyringId KeyID, threadBound bool) {
	// Use the raw syscall to add a key with a fixed type, description, payload,
	// and a specified initial keyring to link it into.
	...
	c.Assert(components, HasLen, 5)
	perms, err := strconv.ParseUint(components[3], 16, 32)
	c.Assert(err, IsNil)
	...
	perms |= uint64(0x08 << 16)
	c.Assert(unix.KeyctlSetperm(id, uint32(perms)), IsNil)

	s.AddKeyToInvalidate(c, KeyID(id), threadBound) // Make sure this key is invalidated during test tear-down.

	// Test the UnlinkKey API.
	c.Check(UnlinkKey(KeyID(id), keyringId), IsNil)
	s.CheckKeyNotInKeyring(c, KeyID(id), keyringId)
}

func (s *keyringSuite) TestUnlinkKey1(c *C) {
	s.testUnlinkKey(c, ProcessKeyring, false)
}
...
func (s *keyringSuite) TestUnlinkKey2(c *C) {
	...
	s.testUnlinkKey(c, ThreadKeyring, true)
}
