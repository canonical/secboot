...
// RecoverFIDO2Salt attempts to recover a secret protected by this keydata's platform
...
func (d *KeyData) RecoverFIDO2Salt() (string, *PlatformKeyData, []byte, error) {
	handlerInfo, exists := keyDataHandlers[d.data.PlatformName]
	if !exists {
		return "", nil, nil, ErrNoPlatformHandlerRegistered
	}
	...
	salt, err := provider.GetSymmetricKey(d.platformKeyData(), nil)
	if err != nil {
		return "", nil, nil, err
	}

	return d.data.PlatformName, d.platformKeyData(), salt, nil
}
