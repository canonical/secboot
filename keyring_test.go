...
func (s *keyringSuite) SetUpTest(c *C) {
	...
	s.JoinSharedAnonymousSessionKeyring(c)
}
...
func (s *keyringSuite) testAddKeyToUserKeyring(c *C, params *testAddKeyToUserKeyringParams) error {
	...
	s.AddKeyToInvalidate(c, added, false)
...
func (s *keyringSuite) testGetKeyFromKernel(c *C, params *testGetKeyFromKernelParams) error {
	...
	if params.unlink {
		// We need user search permissions for the key to be cleaned up
		// automatically in the case where it's unlinked.
		_, _, _, perms, _, err := keyring.DescribeKey(keyring.KeyID(id))
		c.Assert(err, IsNil)

		perms |= keyring.KeyPerm(0x08 << 16)

		c.Assert(keyring.SetPerm(id, perms), IsNil)
	}

	s.AddKeyToInvalidate(c, id, false)
