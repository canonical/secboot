...
func (s *containerSuite) TestContainerActivatePlatform(c *C) {
	...
	err := container.Activate(context.Background(), NewKeyslotInfo(secboot.KeyslotTypePlatform, "foo", 2, 0, nil), key, makeMockActivateConfigGetter(WithVolumeName("data")))
	c.Check(err, IsNil)
...
func (s *containerSuite) TestContainerActivatePlatformDifferentVolume(c *C) {
	...
	container := NewStorageContainer("/dev/nvme0n1p3", unix.Mkdev(259, 3))
	key := testutil.DecodeHexString(c, "a7bfa9a642b897bc13c58b84cf8237d7d1b224b2e63cb3dc10414ff1f9052c56")
	err := container.Activate(context.Background(), NewKeyslotInfo(secboot.KeyslotTypePlatform, "foo", 2, 0, nil), key, makeMockActivateConfigGetter(WithVolumeName("save")))
	c.Check(err, IsNil)
	c.Check(s.commands, DeepEquals, []string{
...
func (s *containerSuite) TestContainerActivatePlatformDifferentKeyslot(c *C) {
	...
	container := NewStorageContainer("/dev/nvme0n1p3", unix.Mkdev(259, 3))
	key := testutil.DecodeHexString(c, "a7bfa9a642b897bc13c58b84cf8237d7d1b224b2e63cb3dc10414ff1f9052c56")
	err := container.Activate(context.Background(), NewKeyslotInfo(secboot.KeyslotTypePlatform, "foo", 1, 0, nil), key, makeMockActivateConfigGetter(WithVolumeName("data")))
	c.Check(err, IsNil)
	c.Check(s.commands, DeepEquals, []string{
...
func (s *containerSuite) TestContainerActivatePlatformNoKeyslotInfo(c *C) {
	...
	container := NewStorageContainer("/dev/nvme0n1p3", unix.Mkdev(259, 3))
	key := testutil.DecodeHexString(c, "a7bfa9a642b897bc13c58b84cf8237d7d1b224b2e63cb3dc10414ff1f9052c56")
	err := container.Activate(context.Background(), nil, key, makeMockActivateConfigGetter(WithVolumeName("data")))
	c.Check(err, IsNil)
	c.Check(s.commands, DeepEquals, []string{
		"Activate(data,/dev/nvme0n1p3,a7bfa9a642b897bc13c58b84cf8237d7d1b224b2e63cb3dc10414ff1f9052c56,-1)",
	})
...
func (s *containerSuite) TestContainerActivateRecovery(c *C) {
	...
	err := container.Activate(context.Background(), NewKeyslotInfo(secboot.KeyslotTypeRecovery, "foo", 3, 0, nil), key, makeMockActivateConfigGetter(WithVolumeName("data")))
	c.Check(err, IsNil)
	c.Check(s.commands, DeepEquals, []string{
...
func (s *containerSuite) TestContainerActivatePlatformMissingVolumeName(c *C) {
	...
	container := NewStorageContainer("/dev/nvme0n1p3", unix.Mkdev(259, 3))
	key := testutil.DecodeHexString(c, "a7bfa9a642b897bc13c58b84cf8237d7d1b224b2e63cb3dc10414ff1f9052c56")
	err := container.Activate(context.Background(), NewKeyslotInfo(secboot.KeyslotTypePlatform, "foo", 2, 0, nil), key, makeMockActivateConfigGetter())
